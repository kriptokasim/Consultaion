...
async def retry_debate_run(
    ...
    from models import DebateStageCheckpoint, Message, Score, Vote
    if not stage_key:
    ...
    if stage_key:
        ...
        stmt_del = (
            sa.delete(DebateStageCheckpoint)
            .where(DebateStageCheckpoint.debate_id == debate_id)
        ...
        if "critique" in stages_to_clear:
            session.execute(sa.delete(Message).where(Message.debate_id == debate_id).where(Message.role == "revised"))
