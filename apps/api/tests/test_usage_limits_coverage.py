...
def test_get_or_reset_counter(db_session, test_user):
    ...
    # Test existing quota within window
    counter.runs_used = 5
...
def test_ensure_daily_token_headroom(db_session, test_user):
    ...
    # Daily token enforcement is intentionally based on the canonical active
    # plan, not the legacy UsageQuota.max_tokens compatibility field.
    counter = _get_or_reset_counter(db_session, test_user.id, "day")
    counter.tokens_used = get_plan_limits("free").daily_token_limit + 1
    db_session.add(counter)
    db_session.commit()
