...
import { ShieldCheck, ShieldAlert, Maximize2, Minimize2, Compass, BookOpen, MoreHorizontal, ClipboardCopy, Download } from "lucide-react"
...
export function DecisionReportShell({
  ...
  React.useEffect(() => {
    ...
    requestAnimationFrame(() => {
      const first = focusTrapRef.current?.querySelector<HTMLElement>(
        'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
      );
      first?.focus();
    });
  ...
  if (isFocusMode) {
    return (
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={title || "Decision Report"}
        className="fixed inset-0 z-50 bg-background dark:bg-stone-950 flex flex-col animate-in fade-in duration-200"
      >
        {/* Sticky top header */}
    ...
    <div className={`space-y-8 ${className || ""}`}>
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {/* Overflow menu — visible on mobile */}
          <div className="relative sm:hidden">
            <button
              onClick={() => setTocOpen(!tocOpen)}
              className="inline-flex items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
            ...
            {tocOpen && (
              <>
                ...
                <div className="absolute right-0 top-full z-20 mt-1 w-44 rounded-xl border border-border bg-card shadow-lg py-1 animate-in fade-in slide-in-from-top-1 duration-100">
                  {showChrome && (
                    <button
                      onClick={() => { setIsFocusMode(true); setTocOpen(false); }}
                      className="flex w-full items-center gap-2 px-4 py-2.5 text-xs font-medium text-foreground hover:bg-muted/50 transition"
                    >
                  ...
                  {onCopy && !isFailed && (
                    <button
                      onClick={() => { onCopy(); setTocOpen(false); }}
                      className="flex w-full items-center gap-2 px-4 py-2.5 text-xs font-medium text-foreground hover:bg-muted/50 transition"
                    >
                  ...
                  {onExport && !isFailed && (
                    <button
                      onClick={() => { onExport(); setTocOpen(false); }}
                      className="flex w-full items-center gap-2 px-4 py-2.5 text-xs font-medium text-foreground hover:bg-muted/50 transition"
                    >
