  ...
  if (!ctx) {
    return {
      pushToast: (toast: Omit<Toast, "id">) => {
        console.warn("pushToast called outside ToastProvider:", toast);
      },
      removeToast: () => {},
    };
  }
...
function ToastMessage({ toast, onRemove }: { toast: Toast; onRemove: (id: number) => void }) {
  useEffect(() => {
    const timer = window.setTimeout(() => onRemove(toast.id), 4500);
