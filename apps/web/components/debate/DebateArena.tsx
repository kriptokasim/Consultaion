...
const statusColors: Record<SSEStatus, string> = {
  ...
  connecting: "bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-200",
        ...
        <aside className="space-y-4 lg:col-span-3 lg:col-start-1">
          <div className="rounded-2xl border border-border bg-card/50 p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-foreground">{t("arena.statusLabel")}</h2>
            ...
            {/* Metrics */}
            <div className="rounded-2xl border border-accent-secondary/20 bg-card p-4 shadow-sm mt-4">
              <div className="flex items-center gap-2 text-sm font-semibold text-accent-secondary">
                <Activity className="h-4 w-4" /> {t("arena.sessionMetrics")}
              </div>
              <dl className="mt-3 space-y-2 text-sm">
                <div className="flex items-center justify-between">
                  <dt className="text-muted-foreground">{t("arena.metrics.rounds")}</dt>
                  <dd className="font-semibold text-foreground">{metrics?.rounds ?? transcriptEvents.length}</dd>
                </div>
                <div className="flex items-center justify-between">
                  <dt className="text-muted-foreground">{t("arena.metrics.tokens")}</dt>
                  <dd className="font-semibold text-foreground">{metrics?.tokensUsed ? metrics.tokensUsed.toLocaleString() : "—"}</dd>
                </div>
                <div className="flex items-center justify-between">
                  <dt className="text-muted-foreground">{t("arena.metrics.elapsed")}</dt>
                  <dd className="font-semibold text-foreground">{formatDuration(metrics?.elapsedSeconds)}</dd>
                </div>
                <div className="flex items-center justify-between">
                  <dt className="text-muted-foreground">{t("arena.metrics.updated")}</dt>
                  <dd className="font-semibold text-foreground">{formatTime(metrics?.updatedAt)}</dd>
                </div>
                {metrics?.budgetReason ? (
                  <div>
                    <dt className="text-xs uppercase tracking-wide text-error">{t("arena.metrics.budget")}</dt>
                    <dd className="text-sm text-error">{metrics.budgetReason}</dd>
                  </div>
                ) : null}
              </dl>
              ...
              <div className="mt-4 border-t border-border pt-4">
                ...
                <div className="space-y-2 text-[11px] font-mono overflow-y-auto max-h-48 pr-1">
                  {logEntries.length === 0 ? (
                    <p className="text-muted-foreground/60">Awaiting telemetry…</p>
                  ) : (
                    logEntries.map((log, idx) => (
                      <p
                        ...
                        className={cn(
                          ...
                          log.level === 'error' && 'text-error font-bold',
                          (!log.level || log.level === 'info') && 'text-muted-foreground'
                        )}
                      >
