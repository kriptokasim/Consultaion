...
export function ModelUsageChart() {
  ...
  if (!rows.length) {
    return (
      <Card className="border-amber-200 bg-white/80 p-4 text-sm text-[#5a4a3a]">
        Model usage data will appear once you run a debate.
      </Card>
    );
  }
  ...
  return (
    <Card className="space-y-3 border-amber-200 bg-white/80 p-4">
      <h4 className="font-semibold text-sm text-[#3a2a1a]">Model usage this period</h4>
          ...
          return (
            <div key={row.model_id}>
              <div className="flex items-center justify-between text-xs text-[#5a4a3a]">
                ...
                <span>{row.tokens_used.toLocaleString()} tokens</span>
              </div>
              ...
              {row.approx_cost_usd != null ? (
                <p className="mt-1 text-[10px] text-stone-500">≈ ${row.approx_cost_usd.toFixed(2)} USD</p>
              ) : null}
            </div>
          );
        })}
